...
viz_popn_bar <- function(in_df, color_scheme, fonts, direction, png_out, width, height){
  ...
  (bar_plot <- ggplot() +
    ...
    reg_part_to_whole_prop <- reg_part_to_whole_prop |> 
    ...
    (bar_plot <- ggplot() +
        ...
        geom_text(data = reg_part_to_whole_prop,
                  aes(x = year, y = pos, label = label_pop),
                  color = "black", size = 3,
                  hjust = 0.5, nudge_x = 0.22#, #-0.23
        ) + #min.segment.length = Inf, label.size = NA
        geom_text(data = reg_part_to_whole_prop,
                  aes(x = year, y = pos, label = sui_label),
                  color = "black", size = 3,
                  hjust = 0.5, nudge_x = 0.3#, #0.21
        )+ #min.segment.length = Inf, label.size = NA
        scale_fill_manual(values = col_pal, 
                          breaks = c("Severe", "High", "Moderate", "Low", "Very low/\nnone"))+
        ylim(c(0, total_bar_height + 10000)) +
