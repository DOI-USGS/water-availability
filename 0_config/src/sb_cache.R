...
#' 
#' @details Following DSP manual, instructions for using ScienceBase programatically, 
#' as of 2/21/2024
initialize_and_cache_sb <- function(sb_username = NULL,
  ...
  if(sbtools::is_logged_in()) {
    ...
    if(override_login) {
      ...
      cli::cli_warn(c(
        "!" = "You are already logged into ScienceBase and re-initialization anyways.",
        "i" = "If you would not like to re-initialize, abort and re-run with {.arg override_login = FALSE}"
      ))
...
update_renviron <- function(sb_username, sb_token, renviron_file) {
  if(file.exists(renviron_file)){
    ...
    if(length(sb_username_idx) > 0) {
      ...
      write(paste0("sb_username=", sb_username), file = renviron_file, append = TRUE)
    }
